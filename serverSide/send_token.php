...
if (!empty($email) && filter_var($email, FILTER_VALIDATE_EMAIL)) {
    ...
    if ($user) {
        ...
        $expiry = date('Y-m-d H:i:s', time() + 20 * 60); // 20 minuti da ora

        $updateStmt = $pdo->prepare("UPDATE studente SET token = :token, token_expiry = :expiry WHERE LOWER(TRIM(mail)) = :email");
        $updateStmt->execute([
            'token' => $token,
            'expiry' => $expiry,
        ...
        try {
            ...
            $mail->SMTPAuth = true;
            $mail->Username = 'elezioni@isistassinari.edu.it';
            $mail->Password = 'hxlm zbzi vqjk fdld';
